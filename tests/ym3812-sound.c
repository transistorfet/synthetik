...
void init_audio()
{
	...
	write_data(0x01, 0x10);

	write_data(0x20, 0x01);		// Set the modulator's multiple to 1
	write_data(0x40, 0x10);		// Set the modulator's level to about 40 dB
	write_data(0x60, 0xF0);		// Modulator attack:  quick;   decay:   long
	write_data(0x80, 0x77);		// Modulator sustain: medium;  release: medium
	write_data(0xA0, 0x98);		// Set voice frequency's LSB (it'll be a D#)
	write_data(0x23, 0x01);		// Set the carrier's multiple to 1
	write_data(0x43, 0x00);		// Set the carrier to maximum volume (about 47 dB)
	write_data(0x63, 0xF0);		// Carrier attack:  quick;   decay:   long
	write_data(0x83, 0x77);		// Carrier sustain: medium;  release: medium
	write_data(0xB0, 0x31);		// Turn the voice on; set the octave and freq MSB
